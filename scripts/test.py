...
class GraspingXarm(object):
    ...
    def __init__(self):
        ...
        self.bottle_class_id = 39  # YOLOv5のCOCOデータセットでのハンドバックのクラスID
        self.confidence_threshold = 0.3  # 50%以上の信頼度
        ...
        self.depth = 0.0  # ボトルの深度情報 
        self.z = 0.20 # depthの目標値
            ...
            for result in results:
                ...
                for box in boxes:
                    ...
                    if cls == self.bottle_class_id and conf > self.confidence_threshold:
                        bottle_list = [map(int, box.xyxy[0]), map(int, box.xyxy[1]), map(int, box.xyxy[2])]
                        x_center_list =[]
                        y_center_list = []
                        w_x_center_list = []
                        w_y_center_list = []
                        l_w_list = []
                        min_l_w = min (l_w_list[0], l_w_list[1], l_w_list[2])
                        for i = 0, i < 3, i++:
                        ...
                        # 深度情報の取得
                        self.depth = self.depth_image[y_center][x_center] / 1000.0  # メートル単位に変換
                        ...
                        if l_w > 0.0064:
                            self.move_xy()
                        elif self.depth > self.z + 10**(-2):
                            plan = None
                            current_pose = self.move_group.get_current_pose().pose

                            plan = self.down_planning(current_pose)
    ...
    def down_planning(self, current_pose):
        ...
        target_pose.position.z = current_pose.position.z - (self.depth - self.z)*0.75
